...
export default function getBehavior() {
    return Behavior({
        ...
        methods: {
            //Triggered when the page is first rendered
            onReady() {
                wx.createSelectorQuery()
                    ...
                    .exec(res => {
                        ...
                        calcSize(info.windowWidth, info.windowHeight*0.9)
            ...
            initVK() {
                ...
                session.start(err => {
                    ...
                    session.on('resize', () => {
                        const info = wx.getSystemInfoSync()
                        calcSize(info.windowWidth, info.windowHeight*0.9, info.pixelRatio)
                    })
                    ...
                    //限制调用帧率
                    let fps = 30 //Render frames per second
                    let fpsInterval = 1000 / fps //The time interval between each frame
                    let last = Date.now() //The elapsed time based on the current time (i.e., the current time minus the previous frame time)
                    ...
                    const onFrame = timestamp => {
                        ...
                        //Request the next animation frame and pass the onFrame function to it as a callback so the animation will loop in a smooth way
            ...
            getRobot() {
                ...
                if (this._insertModels.length > 10) {
                    const needRemove = this._insertModels.splice(0, this._insertModels.length - 10)
